...
CClientKernel * CClientKernel::instance = new CClientKernel();

CClientKernel::CClientKernel()
{
}

CClientKernel * CClientKernel::GetInstance()
{
    return CClientKernel::instance;
}
...
bool CClientKernel::OnMessageHandler(int nHandler,WORD wMain,WORD wSub,unsigned char *pBuffer)
{
	bool result = false;
	do
	{
		if(nHandler != 0)
		{
			...
			if (lua_isfunction(tolua_S, -1))
			{
                ...
                lua_pushstring(tolua_S, (char *)pBuffer);
				int result = LuaEngine::getInstance()->getLuaStack()->executeFunctionByHandler(nHandler, 3)!=0;
			}else{
...
{
    GetMCKernel()->OnMainLoop(GetInstance(), 100);
}
...
void CClientKernel::OnMessageRespon(CMessage* message)
{
	if(message)
	{
		...
		case MSG_SOCKET_CLOSED:
          ...
          */
		case MSG_HTTP_DOWN:
			 OnMessageHandler(message->nHandler,message->wMain,message->wSub,message->pData);
			  break;
		case MSG_UN_ZIP:
