...
static int progressFunc(void *ptr, double totalToDownload, double nowDownloaded, double totalToUpLoad, double nowUpLoaded)
{
    ...
    if (tmpAssets)
    {
        ...
        curl_easy_getinfo(tmpAssets->getCurl(), CURLINFO_SPEED_DOWNLOAD, &speed); // curl_get_info必须在curl_easy_perform之后调用
        ...
        char strSpeed[512]= {0};
        sprintf(strSpeed, "%.f%s/s", round(speed), unit.c_str());
        tmpAssets->upDatePro(tmp, (unsigned char *)strSpeed);
    }
    return 0;
}
...
//更新进度
void CDownAsset::upDatePro(int precent, unsigned char *pData)
{
    if(precent != m_nPrecent)
    {
        ...
        if((m_nPrecent % 2) == 0)
        {
            Notify(DOWN_PRO_INFO,m_nPrecent, pData);
        }
    }
}

//通知UI
void CDownAsset::Notify(int wMain,int wSub, unsigned char *pData)
{
    if (m_nHandler != 0)
    {
        short wSize = 0;
        if (pData)
        {
            wSize = strlen((const char *)pData) + 1;
        }
        GetMCKernel()->HanderMessage(MSG_HTTP_DOWN,m_nHandler,(WORD)wMain,(WORD)wSub, (unsigned char*)pData, wSize);
    }
}
...
void CDownAsset::DownRun()
{
    ...
    do
    {
        ...
        //创建目录
        if (FileUtils::getInstance()->createDirectory(m_szSavePath.c_str()) == false)
        {
